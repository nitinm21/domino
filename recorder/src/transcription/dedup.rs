...
mod tests {
    ...
    fn test_tokenize_apostrophes_stripped() {
        // "it's" -> "it" + "s"; symmetric on both sides, so matches still work.
        assert_eq!(
            tokenize("it's going"),
    ...
    // The session-2026-04-20-1851 regression case: long You segment spanning
    // two consecutive Meeting segments. Must drop when tokens are covered by
    // the union of overlapping meeting segments.
    #[test]
    ...
    // "an input." — 2 tokens, fully covered by the overlapping meeting tail.
    #[test]
    ...
    // Single-token You must be fully covered — exact match required.
    #[test]
    ...
    fn test_window_skew_forward() {
        // Meeting ends at 27.0; You starts at 28.5 with identical text — still
        // within the 3-second lag window, should drop.
    ...
    fn test_window_skew_rejects_late() {
        // Same text but You is 4 seconds past Meeting end — outside window.
    ...
    // Meeting text that's ONLY whisper meta (like `[ Pause ]`) must not be
    // treated as content — a You segment with real words should be kept.
    #[test]
    ...
    fn test_mixed_you_segments_partial_drop() {
        // Real-world shape: first You segment is the user's own speech
        // (kept), second is bleed of a Meeting segment (dropped).
        let you_segs = vec![
    ...
    fn test_is_enabled_default_on() {
        // Env-var test: can be flaky if set globally, so scope to this test.
        // Safety: we save+restore the original value.
